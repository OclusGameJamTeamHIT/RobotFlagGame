...
public class ZRNGUIController : MonoBehaviour {
	...
	void Start () {
		...
		this.GetComponent<CameraController1>().ChangeCamera(0);
		...
		if (menuVisible == true)
		{
			...
			if (operateCameraNumber < 100)
			{
				...
				if (GUI.Button (new Rect (400, 60, 50, 40), "<---"))
				{
					...
					{
						operateCameraNumber = this.GetComponent<CameraController1>().targetCameraNames.Count -1;
				...
				{
					operateCameraNumber++;
					if (operateCameraNumber > this.GetComponent<CameraController1>().targetCameraNames.Count -1)
					{
						operateCameraNumber = 0;
						previousCameraNumber = operateCameraNumber;
					}
				}
				GUI.Label (new Rect (460, 60, 140, 20), this.GetComponent<CameraController1>().targetCameraNames[operateCameraNumber]);
				if (GUI.Button (new Rect (450, 80, 150, 20), "Change"))
				{
					this.GetComponent<CameraController1>().ChangeCamera(operateCameraNumber);
					previousCameraNumber = operateCameraNumber;
					SetQueryChan(0);
				}
			}
